...
def generate_dashboard(
    ...
    for report in reports:
        timeline.append({
            ...
            "date": str(report.get("created_at", report.get("date", "")))
        })
    ...
    for appointment in appointments:
        timeline.append({
            "type": "appointment",
            "title": f"Appointment with {appointment.get('doctor', 'Doctor')}",
            "date": str(appointment.get("created_at", appointment.get("date", "")))
        })

    timeline = sorted(
        [t for t in timeline if t["date"]],
        key=lambda x: x["date"],
        reverse=True
    )
